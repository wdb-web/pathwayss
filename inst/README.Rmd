...
CTSgetR::CTSgetR(str_remove_all(y$Sample," "),"PubChem CID", "KEGG")%>%na.omit()->getkeggnames
...
Description%in%c((n@kegg_analyst$compareClusterResult%>%group_by(Description)%>%
                    summarise(n=n()>1))%>%dplyr::filter(n==T)%>%.$Description)|pvalue     <0.05
)%>%as.data.frame()->f
circos.clear()
...
add.do(x = n@data_mean$a[,n@data_mean$a%>%colnames()%in%get.all.sector.index()]%>%colnames(),
       col= n@data_mean$a[1,n@data_mean$a%>%colnames()%in%get.all.sector.index()]%>%unlist(),scale_height = 0.2)
add.do(x = n@data_mean$a[,n@data_mean$a%>%colnames()%in%get.all.sector.index()]%>%colnames(),
       col= n@data_mean$a[2,n@data_mean$a%>%colnames()%in%get.all.sector.index()]%>%unlist(),scale_height = 0.2)
